...
export function getSourceInfo(skill: Skill): SourceInfo {
  if (skill.storeSourceId) {
    ...
    if (skill.storeSourceId.startsWith('agent:')) {
      return { label: '本地', icon: 'store:local-dir', color: '#8b5cf6', bg: '#ede9fe' }
    }
  ...
  if (skill.source === 'marketplace-json') {
    return { label: 'Marketplace', icon: 'store:marketplace-json', color: '#8b5cf6', bg: '#ede9fe' }
  }
  if (skill.source === 'well-known-index') {
    return { label: 'Well-Known', icon: 'store:well-known-index', color: '#0ea5e9', bg: '#e0f2fe' }
  }
  if (skill.repo) {
