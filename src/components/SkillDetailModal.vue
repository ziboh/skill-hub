...
async function fetchContent() {
  ...
  try {
    ...
    const isWebSkill =
      isWellKnownSkill(props.skill) || (props.skill.source === 'marketplace-json' && !props.skill.repo && props.skill.sourceUrl)
    if (isWebSkill) {
    ...
    if (props.skill.sourceUrl) {
      try {
        const rawUrl = props.skill.sourceUrl.replace('https://github.com/', 'https://raw.githubusercontent.com/').replace('/tree/', '/')
...
async function handleImport() {
  ...
  try {
    ...
    const buffer = await window.services.downloadFile(
      `https://api.github.com/repos/${gh[0]}/${gh[1]}/zipball/${props.skill.branch || 'main'}`,
    ...
    const extractDir = window.services.pathJoin(tempDir, `extract-${skillIdSlug(props.skill.id)}`)
