...
function isInstalled(platformId: string): boolean {
  ...
  return hasSkillMd(record.targetPath)
}
...
function loadInstallStatus() {
  ...
  const valid = allRecords.filter((r) => {
    if (r.targetPath && window.services.pathExists(r.targetPath) && hasSkillMd(r.targetPath)) return true
    ...
    <div v-if="uninstalledPlatforms.length > 0" class="install-toolbar">
      <div class="install-toolbar-row">
        <button
          ...
          :class="{ active: selectedPlatforms.length === totalUninstalled && totalUninstalled > 0 }"
          @click="toggleAllPlatforms"
        >
          <svg
            v-if="selectedPlatforms.length === totalUninstalled && totalUninstalled > 0"
            width="14"
            height="14"
            viewBox="0 0 24 24"
            fill="none"
      ...
      >
        <div class="platform-card-row">
          ...
          <div class="platform-card-info">
            ...
            <span
              v-else-if="physicallyInstalledPlatforms.has(p.id) && selectedPlatforms.includes(p.id)"
...
.platform-card:not(.installed):not(.source):hover {
