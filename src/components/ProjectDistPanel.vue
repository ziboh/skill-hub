...
function _getTargetPath(agentPath: string, project?: RegisteredProject): string {
  const p = project || primaryProject.value
  if (!p) return ''
  return getProjectTargetDir(p.rootDir, agentPath, skillDirName.value)
}
...
function makeRecordKey(agentPath: string, projectId?: string): string {
  ...
  return makeProjectPlatformKey(pid, agentPath)
}
...
function isInstalled(agentPath: string, projectId?: string): boolean {
  ...
  const targetDir = getProjectTargetDir(project.rootDir, agentPath, skillDirName.value)
  if (!window.services.pathExists(targetDir)) return false
  return hasSkillMd(targetDir)
}

const uninstalledAgentDirs = computed(() => agentDirOptions.value.filter((a) => !isInstalled(a.path) && !sourceAgentDirs.value.has(a.path)))
  ...
  try {
    ...
    for (const project of projects) {
      for (const agentPath of agentPaths) {
        const result = deploySkillToProjectPath(props.skill, project, agentPath, props.installMode, {
          sourceDir,
          skipIfRecorded: !showStatusBadges.value,
          verifySkillMd: true,
        })
        const label = `${project.name}/${agentPath}`
        if (result.skipped) {
          addLog(label, 'ok', '已存在，跳过')
          continue
        }
        if (result.ok) {
          installedNames.push(label)
          addLog(label, 'ok', `源: ${sourceDir} → ${result.targetDir}`)
        } else {
          failedMessages.push(`${label}: ${result.message}`)
          addLog(label, 'error', result.message)
        }
      }
    }
...
<template>
  <div class="project-dist-panel">
    ...
    <p class="mode-desc">
      {{
        installMode === 'copy'
          ? '将 SKILL.md 复制到每个平台目录，各平台独立更新。'
          : '创建指向源文件的软链接，所有平台共享同一文件，编辑即时同步。'
      }}
    </p>
    ...
    <div class="project-select-section">
      ...
      <div class="selected-projects-hint">
    ...
    <div v-if="selectedProjects.length" class="install-toolbar">
      <div class="install-toolbar-row">
        <button
          ...
          :class="{ active: selectedAgentDirs.length === totalUninstalled && totalUninstalled > 0 }"
          @click="toggleAllAgentDirs"
        >
          <svg
            v-if="selectedAgentDirs.length === totalUninstalled && totalUninstalled > 0"
            width="14"
            height="14"
            viewBox="0 0 24 24"
            fill="none"
        ...
        <button class="install-all-btn" :disabled="installing || !selectedAgentDirs.length || !selectedProjects.length" @click="install">
          ...
          {{
            installing
              ? installProgressText || '分发中...'
              : selectedProjects.length > 1
                ? `分发到 ${selectedProjects.length} 个项目`
                : '分发所选位置'
          }}
        </button>
      </div>
        ...
        <div class="platform-card-row">
          ...
          <div class="platform-card-info">
            <h4 class="platform-card-name">
              {{ a.name }} <span class="platform-dir-path">/{{ a.path }}</span>
            </h4>
            <template v-if="showStatusBadges">
              ...
              <span
                ...
                class="platform-status-badge existing"
                >覆盖</span
              >
...
.install-all-btn:not(:disabled):hover {
...
.platform-card.installed:hover,
.platform-card.source:hover {
  background: hsl(var(--accent) / 0.15);
  border-color: hsl(var(--border));
  cursor: default;
}
.platform-card.installed {
  border-color: hsl(142 50% 50% / 0.25);
...
.platform-card-name .platform-dir-path {
  font-size: 10px;
  font-weight: 400;
  color: hsl(var(--muted-foreground));
}
...
.platform-status-badge.installed {
  ...
  color: hsl(142 50% 35%);
  border: 1px solid hsl(142 50% 50% / 0.2);
}
.platform-status-badge.source {
  background: hsl(var(--primary) / 0.1);
  color: hsl(var(--primary));
  border: 1px solid hsl(var(--primary) / 0.2);
}
.platform-status-badge.existing {
  background: hsl(38 80% 92%);
  color: hsl(38 90% 35%);
  border: 1px solid hsl(38 80% 50% / 0.2);
}
.platform-status-badge.selected {
  background: hsl(var(--primary) / 0.08);
  color: hsl(var(--primary));
  border: 1px solid hsl(var(--primary) / 0.15);
}
