...
async function deploy() {
  ...
  for (const skill of props.skills) {
    const sourceDir = resolveSkillSourceDir(skill)
    if (!sourceDir) {
    ...
    for (const pid of pids) {
      ...
      const result = deploySkillToGlobalPlatform(skill, platform, installMode.value, sourceDir)
      ...
      <div class="deploy-content">
        ...
        <div v-if="deployProgress" class="deploy-progress">
          <div class="deploy-progress-text">
            ...
            正在分发 {{ deployProgress.skill }} 到 {{ deployProgress.platform }}... ({{ deployProgress.current }}/{{
...
.deploy-platform-card:hover:not(.disabled) {
