...
const {
  addTranslation,
  isTranslating: isTranslatingInQueue,
  findInQueueByHash,
...
const fileHash = computed(() => {
  ...
  if (isDownloaded) {
    const dir = getSkillsRepoDir(props.skill.id)
...
<template>
  <div class="preview-panel space-y-8">
      ...
      <div class="section-header-row">
        ...
        <div class="section-actions">
          ...
          <template v-else>
            ...
            <button
              v-if="descTranslationDone"
              class="heading-btn"
              :disabled="isTranslatingDesc || isPendingDescInQueue"
              @click="handleReTranslateDesc"
            >
              <svg
                width="13"
                height="13"
            ...
            <button
              class="heading-btn"
              :class="{ active: descTranslationDone && showDescTranslation }"
              :disabled="isTranslatingDesc || isPendingDescInQueue"
              @click="handleTranslateDesc"
            >
              <svg
                v-if="isTranslatingDesc || isPendingDescInQueue"
                width="13"
                height="13"
                viewBox="0 0 24 24"
                fill="none"
    ...
    <section class="content-section">
      <div class="section-header-row">
        <h3 class="section-heading mb-0">SKILL 内容</h3>
        <div class="section-actions">
          ...
          <template v-else>
            ...
            <button v-if="translatedContent" class="heading-btn" :disabled="isTranslating || isPendingInQueue" @click="handleReTranslate">
              <svg
                width="13"
                height="13"
                viewBox="0 0 24 24"
            ...
            <button
              class="heading-btn"
              :class="{ active: showTranslation && translatedContent }"
              :disabled="isTranslating || isPendingInQueue"
              @click="handleTranslate"
            >
              <svg
                v-if="isTranslating || isPendingInQueue"
                width="13"
                height="13"
                viewBox="0 0 24 24"
                fill="none"
      ...
      <div class="panel-card content-panel">
        <button
          class="copy-md-btn"
          :title="copyStatus['instr'] ? '已复制' : '复制 MD'"
          @click="emit('copy-content', isEditing ? editedContent : skillContent, 'instr')"
        >
          <svg
            v-if="copyStatus['instr']"
            width="13"
            height="13"
            viewBox="0 0 24 24"
            fill="none"
          ...
          <div v-if="getVisibleContent()" class="skill-markdown-body p-6">
            <template v-if="showTranslation && translatedContent && translationMode === 'immersive'">
              <div v-for="(seg, i) in renderImmersiveSegments(stripFrontmatter(translatedContent))" :key="i">
                <div v-if="seg.type === 'translation'" class="translation-segment">
...
.already-chinese-hint {
  font-size: 12px;
  color: hsl(var(--muted-foreground));
  font-style: italic;
}
.translation-success-badge {
  font-size: 11px;
  font-weight: 500;
  padding: 2px 8px;
  border-radius: 6px;
...
.content-panel:hover .copy-md-btn {
  opacity: 1;
}
.copy-md-btn:hover {
