...
const recordPlatformIds = computed(() => {
  ...
  for (const r of records) {
    if (r.targetPath && window.services.pathExists(r.targetPath)) {
      if (hasSkillMd(r.targetPath)) validIds.add(r.platformId)
    } else {
...
const skillDirName = computed(() => getSkillFolderName(props.skill))
...
function loadProjectInstallStatus() {
  ...
  if (!pids.size) {
    projectDistributeRecords.value = []
    physicallyExistingAgentDirs.value = new Set()
    return
  }
  const valid: any[] = []
  for (const r of allRecords) {
    ...
    for (const pid of pids) {
      if (r.platformId.startsWith(`project:${pid}/`)) {
        matched = true
        break
      }
    }
    if (!matched) continue
    if (r.targetPath && window.services.pathExists(r.targetPath) && hasSkillMd(r.targetPath)) {
      valid.push(r)
      continue
    }
...
const _totalUninstalled = computed(() => uninstalledAgentDirs.value.length)
...
const _selectedProjectCount = computed(() => selectedProjects.value.length)
...
watch(
  () => selectedProjects.value.map((p) => p.id),
  () => {
    loadProjectInstallStatus()
    selectedAgentDirs.value = []
...
async function deployGlobal() {
  ...
  for (const pid of pids) {
    ...
    const result = deploySkillToGlobalPlatform(props.skill, platform, installMode.value, sourceDir)
...
async function deployProject() {
  ...
  for (const project of projects) {
    for (const agentPath of agentPaths) {
      ...
      const result = deploySkillToProjectPath(props.skill, project, agentPath, installMode.value, {
        sourceDir,
        verifySkillMd: false,
      })
      deployResults.value.push({
        platform: `${project.name}/${agentPath}`,
        status: result.ok ? 'ok' : 'error',
        msg: result.message,
      })
      const warning = formatSkillLifecycleWarnings('install', result.warnings)
...
<template>
      ...
      <div class="deploy-content">
        ...
        <template v-if="distScope === 'global'">
          <div class="deploy-section">
            <div class="deploy-section-header">
              ...
              <span class="deploy-section-badge"
                >{{ selectedPlatforms.size }} / {{ platforms.filter((p) => !installedPlatformIds.has(p.id)).length }}</span
              >
            ...
            <div v-else class="deploy-platform-grid">
              ...
              >
                ...
                <div class="deploy-platform-info">
                  ...
                  <span
                    v-else-if="physicallyInstalledPlatforms.has(p.id) && selectedPlatforms.has(p.id)"
                    class="deploy-platform-status existing"
                    >覆盖</span
                  >
                ...
                <button
                  v-if="recordPlatformIds.has(p.id)"
                  class="uninstall-btn"
                  title="卸载"
                  :disabled="deploying"
                  @click.stop="confirmUninstallPlatform(p.id)"
                >
                  <svg
                    width="14"
                    height="14"
                ...
                <div v-else class="deploy-platform-check" :class="{ checked: selectedPlatforms.has(p.id) }">
                  <svg
                    v-if="selectedPlatforms.has(p.id)"
                    width="12"
                    height="12"
                    viewBox="0 0 24 24"
                    fill="none"
        ...
        <template v-else>
          ...
          <div class="deploy-section">
            <div class="deploy-section-header">
              ...
              <button v-if="projectList.length > 1" class="deploy-select-all" :disabled="deploying" @click="toggleAllProjects">
            ...
            <div v-if="selectedProjects.length" class="project-hint">已选 {{ selectedProjects.length }} 个项目</div>
          </div>
          ...
          <div v-if="selectedProjects.length" class="deploy-section">
            ...
            <div class="deploy-platform-grid">
              ...
              >
                ...
                <div class="deploy-platform-info">
                  ...
                  <span
                    v-else-if="physicallyExistingAgentDirs.has(a.path) && selectedAgentDirs.includes(a.path)"
                    class="deploy-platform-status existing"
                    >覆盖</span
                  >
                ...
                <button
                  v-if="isProjectDirInstalled(a.path)"
                  class="uninstall-btn"
                  title="卸载"
                  :disabled="deploying"
                  @click.stop="confirmUninstallItem(a.path)"
                >
                  <svg
                    width="14"
                    height="14"
                ...
                <div v-else class="deploy-platform-check" :class="{ checked: selectedAgentDirs.includes(a.path) }">
                  <svg
                    v-if="selectedAgentDirs.includes(a.path)"
                    width="12"
                    height="12"
                    viewBox="0 0 24 24"
                    fill="none"
            ...
            <div class="custom-dir-row">
              <input
                v-model="customDirInputValue"
                placeholder="自定义目录路径，如 .claude/skills"
                class="custom-dir-input"
                :disabled="deploying"
                @keyup.enter="addCustomDir"
              />
        ...
        <div v-if="deployProgress" class="deploy-progress">
          ...
          <div class="deploy-progress-bar">
            <div class="deploy-progress-fill" :style="{ width: `${(deployProgress.current / deployProgress.total) * 100}%` }" />
          </div>
        </div>
  ...
  <ConfirmModal
    v-if="confirmGlobalUninstall"
    title="卸载 Skill"
    :message="`确定要从该平台卸载 <strong>${props.skill.name}</strong> 吗？`"
    @confirm="uninstallGlobalPlatform"
    @cancel="cancelGlobalUninstall"
  />
  <ConfirmModal
    v-if="confirmUninstall"
    title="卸载 Skill"
    :message="`确定要从项目卸载 <strong>${props.skill.name}</strong> 吗？`"
    @confirm="uninstall"
...
.deploy-select-all:hover {
  opacity: 0.7;
}
.deploy-select-all:disabled {
  opacity: 0.4;
  cursor: default;
}
.deploy-add-project-btn {
  display: flex;
  align-items: center;
  gap: 3px;
  margin-left: auto;
...
.deploy-add-project-btn:hover {
  background: hsl(var(--primary) / 0.2);
}
.deploy-add-project-btn:disabled {
  opacity: 0.4;
  cursor: default;
}
...
.deploy-platform-card:hover:not(.installed):not(.source):not(.disabled) {
...
.deploy-platform-card.installed {
  border-color: hsl(142 50% 50% / 0.2);
  background: hsl(142 50% 50% / 0.03);
  cursor: default;
}
.deploy-platform-card.source {
  border-color: hsl(var(--primary) / 0.2);
  background: hsl(var(--primary) / 0.03);
  cursor: default;
}
.deploy-platform-card.existing {
  border-color: hsl(38 80% 50% / 0.2);
  background: hsl(38 80% 50% / 0.03);
}
.deploy-platform-card.disabled {
  opacity: 0.6;
  cursor: default;
}
.deploy-platform-icon {
  width: 32px;
  height: 32px;
  border-radius: 8px;
  display: flex;
...
.deploy-platform-status.installed {
  color: hsl(142 50% 40%);
}
.deploy-platform-status.source {
  color: hsl(var(--primary));
}
.deploy-platform-status.existing {
  color: hsl(38 90% 45%);
}
.deploy-platform-status.selected {
  color: hsl(var(--primary));
}
.deploy-platform-check {
  width: 20px;
  height: 20px;
  border-radius: 6px;
...
.deploy-platform-check.checked {
  background: hsl(var(--primary));
  border-color: hsl(var(--primary));
  color: #fff;
}
.deploy-platform-check.installed {
  border-color: hsl(142 50% 50%);
  color: hsl(142 50% 40%);
  background: transparent;
}
.deploy-platform-check.existing {
...
.project-select-checkbox.checked svg {
  color: #fff;
}
.project-select-name {
  font-size: 12px;
...
.custom-dir-add-btn:hover:not(:disabled) {
  background: hsl(var(--primary) / 0.2);
}
.custom-dir-add-btn:disabled {
  opacity: 0.4;
  cursor: default;
}
...
.deploy-btn.confirm:hover {
  opacity: 0.9;
}
.deploy-btn.confirm:disabled {
  opacity: 0.5;
  cursor: default;
}
