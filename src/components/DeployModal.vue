      ...
      <div class="deploy-content">
        ...
        <div class="deploy-section">
          <div class="deploy-section-header">
            ...
            <span class="deploy-section-badge">{{ selectedCount }} / {{ platforms.filter(p => !installedPlatformIds.has(p.id)).length }}</span>
            <button class="deploy-select-all" @click="toggleAll" :disabled="deploying">
              {{ selectedCount === platforms.filter(p => !installedPlatformIds.has(p.id)).length ? '取消全选' : '全选' }}
            </button>
          </div>
          ...
          <div v-else class="deploy-platform-grid">
            ...
            >
              ...
              <div v-if="recordPlatformIds.has(p.id) || sourcePlatformIds.has(p.id)" class="deploy-platform-check installed">
...
.deploy-modal { width: 520px; max-width: 90vw; max-height: 85vh; background: hsl(var(--card)); border: 1px solid hsl(var(--border)); border-radius: 20px; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 24px 64px hsl(0 0% 0% / 0.2); }
